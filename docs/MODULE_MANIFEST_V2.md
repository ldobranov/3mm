...
`data.write`, `events.publish`, `network.outbound`, `process.spawn`,
`hardware.inventory`, and `hardware.gpio`. Stronger OS-level isolation remains
required before untrusted generated code is supported.
