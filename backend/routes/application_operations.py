...
@router.post('/{module_id}/jobs/{job_id}/resolve')
def resolve_application_job(module_id: str, job_id: str, request: JobResolutionRequest,
                            admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    installation = _installation(db, module_id)
    locked = db.execute(update(ApplicationExtensionInstallation).where(
        ApplicationExtensionInstallation.id == installation.id,
        ApplicationExtensionInstallation.enabled.is_(False),
        ApplicationExtensionInstallation.status == 'disabled')
        .values(updated_at=ApplicationExtensionInstallation.updated_at)).rowcount
    if not locked:
        db.rollback()
        raise HTTPException(409, 'Disable the application and review its external outcomes first')
    state = db.scalar(select(ApplicationJobState).where(
        ApplicationJobState.application_installation_id == installation.id,
        ApplicationJobState.job_id == job_id))
    if state is None or state.lease_token is None:
        db.rollback()
        raise HTTPException(409, 'Job has no unresolved claim')
    # Invalidate the old executor's completion token as well as its schedule.
    state.lease_token = state.lease_until = None
    state.last_outcome, state.last_error = 'resolved', None
    state.next_run_at = datetime.now(UTC)
    db.add(AuditLog(user_id=admin.id, action='APPLICATION_JOB_RESOLVED',
        entity_type='application_extension', entity_name=module_id, changes={'job_id': job_id}))
    db.commit()
    return {'job_id': job_id, 'status': 'resolved'}
