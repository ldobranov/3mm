  ...
  methods: {
    async fetchMetrics() {
      try {
        ...
        const response = await fetch('/api/extensions/hiveos/api/system/metrics/current');
    ...
    async fetchHistoricalData() {
      try {
        const response = await fetch(`/api/extensions/hiveos/api/system/metrics/history?hours=${this.config.historyHours || 24}&interval=60`);
